...
namespace Orchard.Tokens.Tests {
	...
	public class ListTokenTests {
	    ...
	    [Test]
	    public void TestListForEachTokens()
	    {
            Assert.That(_tokenizer.Replace("{List.ForEach:{Content.Id} }", new { List = new List<IContent> { new TestUser { Id = 5 }, new TestUser { Id = 10 } } }), Is.EqualTo("5 10 "));
	    }
	    ...
	    public void TestListTokensAdhereToEncodings()
	    {
            Assert.That(_tokenizer.Replace(
                "{List.ForEach:<strong>{Content.Id}<strong>}", 
                new { List = new List<IContent> { new TestUser { Id = 5 }, new TestUser { Id = 10 } } }, new ReplaceOptions { Encoding = ReplaceOptions.HtmlEncode }), 
                Is.EqualTo("&lt;strong&gt;5&lt;strong&gt;&lt;strong&gt;10&lt;strong&gt;"));
            Assert.That(_tokenizer.Replace(
                "{List.ForEach:<strong>{Content.Id}<strong>}", 
                new { List = new List<IContent> { new TestUser { Id = 5 }, new TestUser { Id = 10 } } }, new ReplaceOptions { Encoding = ReplaceOptions.NoEncode }), 
                Is.EqualTo("<strong>5<strong><strong>10<strong>"));
	    }
